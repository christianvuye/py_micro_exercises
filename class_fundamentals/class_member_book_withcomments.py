...
class Member:
    ...
    def reserve_book(self, book):
        ...
        if book.status != "available": # so the book is reserved or borrowed already, you dont need to check if the status is reserved or borrowed, this does it
            return False
        
        if book not in self.reserved_books and book not in self.borrowed_books: # check again that it is not reserved or borrowed, (so it is available)
            self.reserved_books.append(book)                                    # but this time in the list AND for this particular member instance! 
    ...
    def borrow_book(self, book):
        ...
        if book in self.borrowed_books or book.status == "borrowed": # check if this member has already borrowed the book
            return False                                             # or it could be in another members borrowed book list 

        if book in self.reserved_books and book.status == "reserved": # book is in this members reserved list and is reserved by that same member
            self.reserved_books.remove(book)
            self.borrowed_books.append(book)
            book.status = "borrowed"
            return True
    
        if book.status == "reserved": return False # this book is reserved by some other member so this member instance cannot borrow it
    ...
    def return_book(self, book_title): # no type checking needed here, if the book title is not present nothing will happen - could add an else
        for book in self.borrowed_books: # this already checks if the book is in THIS members borrowed book list
...
class Book:
    ...
    def _validate_status(status):
        allowed_status = ("available", "borrowed", "reserved") # tuple, so it is immutable & cannot be changed anywhere
        if status not in allowed_status:
            raise ValueError(f"Status should be one of the following: {allowed_status}")
        return status
    
    def reserve_by(self, member): # member or member name? Member object makes it easier so lets keep it this way

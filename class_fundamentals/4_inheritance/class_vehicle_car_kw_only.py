...
class Vehicle:
    def __init__(self, make, model, year=2024, color="white", mileage=0):
        self.make = make
        self.model = model
        self.year = self._validate_year(year) # using self here bcse the static method is a part of the class
    ...
    def _validate_mileage(mileage):
        if not mileage >= 0: # no reason for storing zero in a variable, mileage won't change and can't logically be under 0
            raise ValueError(f"Mileage cannot be negative.")
        else:
            return mileage
...
class Car(Vehicle):
    def __init__(self, make, model, year=2024, color="white", mileage=0, doors=4, fuel_type="gasoline"):
        year = Car._validate_year(year) # better to explicitely call Car prefix to make clear it is static method of Car not Vehicle class
    ...
    def _validate_fuel_type(fuel_type):
        ...
        if not fuel_type in allowed_fuel_types:

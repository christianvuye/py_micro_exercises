...
class Employee:
    def __init__(self, name, employee_id, salary=50000, start_date="2024-01-01", department="General"):
        self.name = name
    ...
    def _validate_employee_id(employee_id):
        if not isinstance(employee_id, int): # because of conversion to str for len(), input could be 6 word str and pass validation
...
class Manager(Employee):
    def __init__(self, name, employee_id, salary=80000, start_date="2024-01-01", team_size=5, bonus_percentage = 0.1): # raised default salary to minimum value
        salary = Manager._validate_manager_salary(salary)
        Employee.__init__(self, name, employee_id, salary, start_date, department="Management")
    ...
    def _validate_team_size(team_size):
        ...
        if not team_size > 0:
            raise ValueError(f"Team size should be above 0")
        else:
            return team_size
